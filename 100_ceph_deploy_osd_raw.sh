...
${PARTED} -s -a optimal /dev/${CDISK} mklabel gpt
${PARTED} -s -a optimal /dev/${CDISK} mkpart primary 0% 100%
${PARTED} /dev/${CDISK} -s name 1 "bluestore_data_for_${CDISK}"
...
${CEPHVOLUME} lvm create --bluestore --data /dev/${CDISK}1 --block.wal ${BLOCKWALVG}/blockwal_for_${CDISK} --block.db ${BLOCKWALVG}/blockdb_for_${CDISK}
